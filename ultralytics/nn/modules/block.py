...
from .attention import ECA, CBAM, CoordAtt
from .attention_refined import MultiAttention_ECA_CBAM, MultiAttention_Triple
...
# Custom Attention-Enhanced Blocks
class C2f_ECA(nn.Module):
    """
    C2f block with Efficient Channel Attention (ECA) module.
    
    This block combines the efficient feature extraction of C2f with ECA's lightweight
    channel attention mechanism. ECA captures cross-channel interactions with minimal
    computational overhead, making it ideal for resource-constrained applications.
    
    Architecture:
    Input -> Conv1x1 -> [Bottleneck] x n -> Conv1x1 -> ECA -> Output
    ...
    Args:
        ...
        g (int): Groups for grouped convolution in bottlenecks. Default: 1
    ...
    def __init__(self, c1: int, c2: int, n: int = 1, shortcut: bool = False, 
        ...
        # ECA attention applied to output channels
        self.eca = ECA(c2)
        
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Forward pass of C2f_ECA block.
        
        Args:
            x (torch.Tensor): Input tensor of shape (B, c1, H, W)
            
        Returns:
            torch.Tensor: Output tensor of shape (B, c2, H, W) with ECA attention applied
        """
        # Input projection and split
        y = list(self.cv1(x).chunk(2, 1))
        
        # Progressive feature extraction through bottlenecks
        y.extend(m(y[-1]) for m in self.m)
...
class C2f_CBAM(nn.Module):
    """
    C2f block with Convolutional Block Attention Module (CBAM).
    
    This block enhances C2f with CBAM's dual attention mechanism that sequentially
    applies channel and spatial attention. CBAM helps the network focus on both
    'what' and 'where' to attend in the feature maps.
    
    Architecture:
    Input -> Conv1x1 -> [Bottleneck] x n -> Conv1x1 -> CBAM -> Output
    ...
    def __init__(self, c1: int, c2: int, n: int = 1, shortcut: bool = False,
        ...
        # CBAM attention applied to output channels
        self.cbam = CBAM(c2)
        
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Forward pass of C2f_CBAM block.
...
class C2f_CoordAtt(nn.Module):
    ...
    This block integrates Coordinate Attention into C2f, which factorizes channel attention
    into two 1D operations along spatial dimensions. This allows the model to capture
    long-range dependencies with precise positional information while being mobile-friendly.
    ...
    def __init__(self, c1: int, c2: int, n: int = 1, shortcut: bool = False,
                 g: int = 1, e: float = 0.5, reduction: int = 32) -> None:
        super().__init__()
        ...
        # Coordinate attention applied to output channels
        # Note: inp and oup are both c2 for residual connection
        self.coordatt = CoordAtt(c2, c2, reduction=reduction)
        
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Forward pass of C2f_CoordAtt block.
...
class C2f_ECA_CBAM(nn.Module):
    """
    C2f block with sequential ECA + CBAM multi-attention.
    ...
    Args:
        ...
        shortcut (bool): Add shortcut connection. Default: False
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        # Sequential ECA + CBAM multi-attention
        self.multi_attention = MultiAttention_ECA_CBAM(c2)

    def forward(self, x):
        """Forward pass with ECA + CBAM multi-attention."""
        y = list(self.cv1(x).chunk(2, 1))
        y.extend(m(y[-1]) for m in self.m)
        x = self.cv2(torch.cat(y, 1))
        
        # Apply sequential ECA + CBAM attention
        x = self.multi_attention(x)
        
        return x


class C2f_Triple_Attention(nn.Module):
    """
    C2f block with hierarchical triple attention (ECA + CBAM + CoordAtt).
    
    Ultimate attention configuration combining all three mechanisms:
    - ECA for efficiency and initial channel focus
    - CBAM for comprehensive dual attention 
    ...
    Args:
        ...
        g (int): Groups for convolutions. Default: 1
        e (float): Channel expansion ratio. Default: 0.5
        attention_dropout (float): Attention dropout rate. Default: 0.1
    """
    
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, attention_dropout=0.1):
        super().__init__()
        self.c = int(c2 * e)
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv((2 + n) * self.c, c2, 1)
        ...
        # Hierarchical triple attention
        self.multi_attention = MultiAttention_Triple(c2, attention_dropout=attention_dropout)
