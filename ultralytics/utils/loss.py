...
def eiou_loss(box1, box2, xywh=True, eps=1e-7):
    """
    Calculate Efficient IoU Loss (EIoU).
    
    EIoU addresses the ambiguity problem in IoU-based losses by considering the difference of 
    width and height directly. Particularly effective for small, linear defects like those 
    in HRIPCB dataset.
    
    Paper: "Focal and Efficient IOU Loss for Accurate Bounding Box Regression"
    
    Args:
        box1 (torch.Tensor): Predicted boxes in format (x1, y1, x2, y2) or (x, y, w, h)
        box2 (torch.Tensor): Ground truth boxes in same format
        xywh (bool): If True, boxes are in (x, y, w, h) format, else (x1, y1, x2, y2)
        eps (float): Small epsilon to avoid division by zero
        
    Returns:
        torch.Tensor: EIoU loss values
    """
    ...
    # EIoU = IoU - rho2/c2 - rho_w2 - rho_h2
    return 1 - iou + rho2 / c2 + rho_w2 + rho_h2


def siou_loss(box1, box2, xywh=True, eps=1e-7):
    """
    Calculate Scylla IoU Loss (SIoU).
    
    SIoU redefinies the penalty metrics by considering the vector angle between the required 
    regression direction and the direction to the center point. Excellent for datasets with 
    small, linear defects where direction matters.
    ...
    # Angle cost  
    s_cw = (b2_x + b1_x - cw + eps) / 2
    s_ch = (b2_y + b1_y - ch + eps) / 2
    ...
    theta = 4 / (math.pi ** 2)
    shape_cost = theta * (omiga_w ** 2 + omiga_h ** 2)
    
    # SIoU = 1 - IoU + Distance_Cost + Angle_Cost + Shape_Cost
    return 1 - iou + (rho2 / c2) + angle_cost + shape_cost
